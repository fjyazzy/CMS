Public Class Cxbitem
    ...
    Private Sub getdH(ByVal dh As String)
        ...
        rs.Open("select * from cxbjb where id=" & dh, Conn, 1, 1)
        If rs.EOF Then
            Txtdh.Text = "单号不存在！"
            Button1.Enabled = False
            Button1.Text = dh & ":单号不存在！"
        Else
            Txtdh.Text = rs.Fields("cxbjbh").Value
        End If
        rs.Close()
    End Sub
    ...
    Protected Sub fillForm(ByVal id As String)
        If CInt(xID) > 0 Then
            ...
            If Not rs.EOF Then
                ...
                Txtmc.Text = rs.Fields(3).Value
                Txtdw.Text = rs.Fields(4).Value
                TxtSl.Text = rs.Fields(5).Value
                Txtdj.Text = rs.Fields(6).Value
                TxtJe.Text = rs.Fields(7).Value
                Txtbz.Text = rs.Fields(8).Value
    ...
    Protected Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        rs.Fields(3).Value = Txtmc.Text
        rs.Fields(4).Value = Txtdw.Text
        rs.Fields(5).Value = TxtSl.Text
        rs.Fields(6).Value = Txtdj.Text
        rs.Fields(7).Value = TxtJe.Text
        rs.Fields(8).Value = Txtbz.Text
        ...
        rs.Open("select sum(je) from cxbjb2 where cxbjbh='" & Txtdh.Text & "'", Conn, 1, 3)
        zj = rs.Fields(0).Value
        rs.Close()
        Conn.Execute("update cxbjb set zj =" & zj & " where cxbjbh='" & Txtdh.Text & "'")
